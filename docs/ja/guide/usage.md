...
```text
赫狼レグルス / 境界を渡る導線術士: ランタイムのクロスプラットフォーム化
...
- subagent 1: docs drift review（producer。subagent 2 を peer verify）
- subagent 2: docs example consistency scan（producer。subagent 1 を peer verify）
- subagent 3: Material Design 特化デザイナー（user-facing docs/UI audit。`material_design_status` を返す）
